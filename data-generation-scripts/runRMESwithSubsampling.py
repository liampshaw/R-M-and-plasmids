...
def main():
    ...
    if bases_subsample==0: # Define '0' as 'no subsampling'
        runRMES(fasta, output, k)
    else:
        tmp_fasta_for_splitting = makeTmpFile(fasta, suffix='_compatible.fa')
        makeCompatibleFasta(fasta, tmp_fasta_for_splitting) # Make fasta RMES compatible so we can split it  
        sequence_chunks = splitFasta(tmp_fasta_for_splitting, split_length=bases_subsample)
        if os.path.exists(tmp_fasta_for_splitting):
            os.remove(tmp_fasta_for_splitting) # Remove that tmp fasta
