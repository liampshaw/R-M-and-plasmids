...
def main():
    ...
    files = glob.glob(input_dir+'/*/*RMS-merged.csv')
    print(input_dir)
    ...
    # Read in all of the targeted motifs
    target_rs_dict = readTargetedRS(files)
    ...
    for k in k_values:
        print(k)
        saveKmerDB(k, target_rs_dict, output_dir+str(k)+'-kmer-db.csv')
        ...
        for s in answers.keys():
            ...
            for kmer in kmers_of_interest: 
                ...
                if answer!="unknown":
                    print(s, kmer, answer)    
        with open(output_dir+str(k)+'-species-target-dict.pkl', 'wb') as db_handle:
